...
def main():
    ...
    likelihood = full_likelihood(args.dedicated)
    ...
    dedicated_score = likelihood( features, bsm_point)
    fig, ax = plt.subplots(1, 1, figsize=[14,8])
    dedicated_sm,bins,_  = ax.hist(dedicated_score.detach().numpy(), weights=sm_weight.detach().numpy(),
                                   bins=200 , alpha=0.5, label="SM" , density=True)
    dedicated_bsm,_   ,_ = ax.hist(dedicated_score.detach().numpy(), weights=bsm_weight.detach().numpy(),
                                   bins=bins, alpha=0.5, label="BSM", density=True)
    ax.legend()
    fig.savefig(f"{args.name}/hist_dedicated.png")
    fig.clf()
    
    likelihood = full_likelihood(args.parametric)
    bsm_point = args.bsm_point.replace("=","_").replace(":","_").split('_')
    bsm_point = dict(map(lambda i: (bsm_point[i], float(bsm_point[i+1])), range(len(bsm_point)-1)[::2]))
    ...
    roc_dedi, auc_dedi, a_dedi = net_eval(dedicated_score, sm_weight, bsm_weight)
